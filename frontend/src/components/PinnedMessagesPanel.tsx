      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {!isLoading && pins.length === 0 && (
          <div className="text-center py-8">
            <Icon name="pin" size={32} className="text-sp-muted mx-auto mb-2" />
            <p className="text-sp-muted text-sm">No pinned messages yet.</p>
            <p className="text-sp-muted text-xs mt-1">Pin important messages using the message toolbar.</p>
          </div>
        )}
        {pins.map((pin) => (
          <div key={pin.id} className="bg-sp-input rounded-sp-md p-3 group relative">
            {/* Pinned-by line */}
            <div className="flex items-center gap-1.5 mb-2 text-xs text-sp-muted">
              <Icon name="pin" size={11} className="text-sp-mention" />
              <span>
              ...
              <button
                className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity text-sp-muted hover:text-red-400"
                title="Unpin"
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline gap-2 mb-0.5">
                  <span className="text-sm font-semibold text-sp-text">{pin.message.author.username}</span>
                  <span className="text-[10px] text-sp-muted">{format(new Date(pin.message.created_at), 'HH:mm')}</span>
                </div>
                <p className="text-sm text-sp-text leading-snug line-clamp-4 break-words">
                  {pin.message.is_deleted
                    ? <em className="text-sp-muted">Message was deleted</em>
                    : pin.message.content}
                </p>
              </div>
            </div>
            ...
            {onScrollToMessage && !pin.message.is_deleted && (
              <button
                className="mt-2 text-[11px] text-sp-mention hover:underline"
