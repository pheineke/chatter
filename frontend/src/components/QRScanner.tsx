...
export function QRScanner({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="relative w-full max-w-md rounded-2xl bg-discord-sidebar p-6 shadow-2xl">
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute right-4 top-4 text-discord-muted hover:text-white"
        >
          <Icon name="x" size={20} />
        </button>
          ...
          <div className="space-y-4">
            <div className="relative overflow-hidden rounded-xl bg-black aspect-[4/3]">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="h-48 w-48 rounded-lg border-2 border-discord-mention/70 shadow-[0_0_0_9999px_rgba(0,0,0,0.5)]" />
              </div>
            </div>
            <canvas ref={canvasRef} className="hidden" />
            <p className="text-center text-sm text-discord-muted">
        ...
        {phase === 'confirming' && scannedPayload && (
          <div className="space-y-5 text-center">
            <div className="flex items-center justify-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-discord-mention/20">
                <Icon name="monitor" size={32} className="text-discord-mention" />
              </div>
            </div>
            <div>
            ...
            <div className="flex gap-3 justify-center">
              <button
                ...
                className="rounded px-4 py-2 text-sm text-discord-muted hover:bg-discord-input transition"
              >
                Cancel
              </button>
              <button
                onClick={handleApprove}
                className="rounded bg-discord-mention px-6 py-2 text-sm font-semibold text-white hover:bg-indigo-500 transition"
              >
                Approve
              </button>
            </div>
        ...
        {phase === 'approving' && (
          <div className="flex flex-col items-center gap-4 py-6">
            <div className="animate-spin rounded-full h-10 w-10 border-2 border-discord-mention border-t-transparent" />
            <p className="text-discord-muted text-sm">Transferring keys…</p>
          </div>
        )}

        {phase === 'done' && (
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            <Icon name="check-circle" size={40} className="text-discord-online" />
        ...
        {(phase === 'error' || phase === 'no-camera' || phase === 'no-key') && (
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            ...
            <button
              onClick={onClose}
              className="rounded bg-discord-input px-6 py-2 text-sm font-semibold text-discord-muted hover:text-white transition"
            >
              Close
            </button>
          </div>
